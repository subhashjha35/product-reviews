...
# clean strings
text = [helper.clean_string(t) for t in text]
...
writer = tf.summary.FileWriter("./log", graph=sesh.graph)

if restore:
    saver.restore(sesh, "log/lstmmodel.ckpt")
...
def train_model(X_train, y_train):
    for epoch in range(num_epochs):
        ...
        for i in range(num_batches):
            ...
            _, l, a = sesh.run([optimizer, loss, accuracy],
                                     feed_dict={x: x_batch, y: y_batch})

            if i > 0 and i % 100 == 0:
                print("STEP", i, "of", num_batches, "LOSS:", l, "ACC:", a)
            if i > 0 and i % 500 == 0:
                saver.save(sesh, "log/lstmmodel.ckpt")
                writer.flush()
                writer.close()
        saver.save(sesh, "log/lstmmodel.ckpt")
        writer.flush()
...
def predict_custom(sentences):
    ...
    stop = helper.stopwords.words('english') + list(helper.string.punctuation)

    text_clean = []

    for s in sentences:
        text_clean.append(" ".join([i for i in helper.word_tokenize(s.lower()) if i not in stop and i[0] != "'"]))
...
def home():
  ...
  print(req_data)
  req_text = req_data['reviewText']
  result = predict_custom(req_text)
  print(req_text, result)
  return jsonify({'reviewScore': int(result)})
